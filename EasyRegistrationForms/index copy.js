                ...
                db.collection('submissions').add(submissionData)
                    .then((docRef) => {
                        ...
                        if (freeSubmissions === 0) {
                            ...
                            // Add transaction record with regular Date object
                            const transaction = {
                                type: 'debit',
                                amount: registrationCost,
                                description: `Business registration (${getRegistrationTypeName(selectedRegistrationType)})`,
                                timestamp: new Date(), // FIX: Use regular Date instead of serverTimestamp
                                ref: docRef.id
                            };
...
// ✅ CORRECTED: processPayment function
function processPayment() {
    ...
    const handler = PaystackPop.setup({
        ...
        callback: function (response) {
            ...
            // Use a regular JavaScript Date object instead of serverTimestamp()
            const transaction = {
                type: 'credit',
                amount: amount,
                method: 'Mobile Money',
